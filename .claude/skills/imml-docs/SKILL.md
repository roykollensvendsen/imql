...
name: imml-docs
description: >-
  Build, preview, and deploy the IMML documentation site (Material for MkDocs, Qt-style). Use when the
...
# imml-docs — the documentation site
...
- **Hand-written** (reuse `spec/` via `--8<--` snippets, base_path `.`): the guide and language pages
  ...
  `docs/index.md` and `docs/guides/quick-start.md`. **Keep `.imml` examples canonical** — they must
...
- **Generated** by `docs/gen/build.py` (do NOT hand-edit generated pages):
  ...
  - `examples/*` — all 189 subnets lifted via `imml_core.lift` (IMML/IR tabs), grouped in nav by
...
version label tracks `schema/VERSION`. Live at **https://roykollensvendsen.github.io/imml/** (Pages is
