...
name: imql-dev
description: >-
  Develop and extend the IMQL language (this repo): the schema/IR, the lark grammar + lift/compile
...
# imql-dev — developing the IMQL language

IMQL is a declarative language for Bittensor incentive mechanisms. This repo holds the schema (the IR),
the grammar + round-trip tooling, a metric ontology, a generator, a 189-subnet corpus, and a docs site.
Read `spec/04-imql-language.md` and `spec/05-imql-style.md` first.
...
./.venv/bin/python tooling/fmt.py --check <lifted .imql>                            # canonical
./.venv/bin/mkdocs build --strict                                                  # docs clean
```

## Hard invariants (do not break)
- **Round-trip fidelity = 100%.** `coverage.py` lifts every IR to IMQL, compiles back, and compares the
  **structural signature** (`imql_core.signature` — enums, params, typed nodes, composition; it
...
- `tooling/imql_core.py` — the heart: `signature()`, `lift()` (IR→IMQL), `compile_text()` (IMQL→IR, lark
...
- `tooling/{lift,compile,coverage,fmt,canonicalize,generate,derive-composition,validate,stress-report}.py`
  — CLIs (thin wrappers over imql_core + the schema).
- `vocab/metric-ontology.yaml` — 3-level metric vocab (raw→specific→family); single source for the IR's
  `metric_family` enum AND the IMQL metric types.
- `lang/imql.ebnf` — the documented grammar. **KNOWN DIVERGENCE:** the EBNF describes a richer surface
  (`from groundtruth`, `metric family(specific)`) that the *live* lark grammar in `imql_core.py` does NOT
...
- `docs/` + `mkdocs.yml` — see the `imql-docs` skill.
...
## Backlog (next work)
1. **Reconcile `lang/imql.ebnf` with the live parser** — either implement the richer surface
...
4. **Mechanism simulator (the improvement arc)** — instantiate an IMQL spec against strategic miner
...
## Related
- `imql-docs` skill — build/preview/deploy the documentation site.
